...
public class Ai2 : MonoBehaviour
{
    ...
    void Awake()
    {
        ...
        else if (rnd > 6) { sidetarget = "right"; }
        else if(rnd > 4) { sidetarget = "left"; }
    ...
    public void CanSeePlayer(GameObject curtarget)
    {
        ...
        Vector3 targetspot = curtarget.transform.position;
        //    targetRotation = Quaternion.LookRotation(patroltarget.transform.position - transform.position);
        if (sidetarget == "up") { targetspot = curtarget.transform.position + (curtarget.transform.up * closedistance); }
        else if (sidetarget == "down") { targetspot = curtarget.transform.position + (curtarget.transform.up * -closedistance); }
        else if (sidetarget == "right") { targetspot = curtarget.transform.position + (curtarget.transform.right * closedistance); }
        else if (sidetarget == "left") { targetspot = curtarget.transform.position + (curtarget.transform.right * -closedistance); }
        else { targetspot = curtarget.transform.position + (curtarget.transform.forward * -closedistance); }

       

        if (Vector3.Distance(transform.position, targetspot) > fardistance)
        { rb.velocity = Vector3.Lerp(rb.velocity, transform.forward * speed, Time.deltaTime); }
        else {
            if (Vector3.Distance(transform.position, targetspot) > closedistance * 0.1f)
            { rb.velocity = Vector3.Lerp(rb.velocity, (targetspot - transform.position).normalized * speed, Time.deltaTime * acceleration); }
