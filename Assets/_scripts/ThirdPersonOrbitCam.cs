...
public class ThirdPersonOrbitCam : MonoBehaviour 
{
	...
	void LateUpdate()
	{
        ...
        if (Mathf.Abs(transform.eulerAngles.y - player.eulerAngles.y) > 10)
        {
            angleH += Mathf.Clamp(Input.GetAxis("4th Axis"), -1, 1) * horizontalAimingSpeed * Time.deltaTime;
        }

        if (Mathf.Abs(transform.eulerAngles.x - player.eulerAngles.x) > 10)
        {
            angleV -= Mathf.Clamp(Input.GetAxis("5th Axis"), -1, 1) * verticalAimingSpeed * Time.deltaTime;
        }
        if (Mathf.Abs(transform.eulerAngles.x - player.eulerAngles.x) > 10)
        {
            angleZ -= Mathf.Clamp(Input.GetAxis("5th Axis"), -1, 1) * verticalAimingSpeed * Time.deltaTime;
        }
