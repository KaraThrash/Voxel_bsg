...
public class TurningShip : MonoBehaviour
{
...
//each ship type has its core values to modify the player stats
  ...
  public void Fly (Rigidbody shipRigidBody) {
    ...
    public void ControlCamera(GameObject cam,GameObject ship)
    {
      ...
      {
        targetRotation = Quaternion.LookRotation(ship.transform.position - (camera.transform.position + camera.transform.forward) );
        // step = Mathf.Min(4 * Time.deltaTime, 1.5f);
        ship.transform.rotation = Quaternion.Lerp(ship.transform.rotation, targetRotation, Time.deltaTime  * turnSpeed);
      }
      else {
    ...
    public void thirdpersonflightcontrols(Rigidbody shipRigidBody)
    {
      ...
      {  vert = Input.GetAxis("Vertical");}else{vert = 0;}
        ...
        if(rollMod < 1){rollMod = 1;}
              ...
              { lift = 1; }
              else  if  ((Input.GetKey(KeyCode.LeftShift) ) && (shipRigidBody.velocity.magnitude < 5 || playerControls.UseStamina(1) == true))
                { lift = -1; }
              else { lift = 0; }
        ...
        shipRigidBody.velocity = Vector3.Lerp(shipRigidBody.velocity,transform.forward * flySpeed * lift * rollMod,2.0f * Time.deltaTime);
          shipRigidBody.transform.Rotate(vert * rollSpeed  * Time.deltaTime,hort * rollSpeed * Time.deltaTime,roll * rollSpeed  * Time.deltaTime);
    }
