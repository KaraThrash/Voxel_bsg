  ...
  public void Fly (Rigidbody shipRigidBody) {
    ...
    public void ControlCamera(GameObject cam,GameObject ship)
    {
      ...
      if (Input.GetMouseButton(1))
      {
        ...
        turret.transform.rotation = Quaternion.Lerp(turret.transform.rotation, cam.transform.rotation, Time.deltaTime * rollMod * turnSpeed);
...
//TODO: change this '20' to a reasonable variable // cam should be slightly behind ship rotation to give the semse of movement
    ...
    public void thirdpersonflightcontrols(Rigidbody shipRigidBody)
    {
      if(Input.GetAxis("Horizontal") != 0 )
      {
        ...
        if(Input.GetAxis("Horizontal") > 0 )
        {
          body.transform.Rotate(transform.up * turnSpeed * Time.deltaTime);

        }else{  body.transform.Rotate(transform.up * -turnSpeed * Time.deltaTime);}
      }
      else{

        if(Input.GetAxis("Vertical") != 0 )
        {  shipRigidBody.AddForce(body.transform.forward * flySpeed * Input.GetAxis("Vertical") * Time.deltaTime,ForceMode.Impulse);}
        else{
          vert = 0;
        ...
        if (Input.GetMouseButton(0))
        {
            ...
            if (guncooldowntimer <= 0)
            {
              ...
              shipRigidBody.AddForce(body.transform.up * flySpeed *  Time.deltaTime,ForceMode.Impulse);
